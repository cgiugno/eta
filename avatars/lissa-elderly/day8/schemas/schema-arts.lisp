...
;; *have-arts-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-arts-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-arts-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-arts-dialog*

'(event-schema :header (((set-of ^me ^you) have-arts-dialog.v) ** ?e)
;`````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(Have you ever taken lessons in music\, dance\, theatre\, drawing\, or another kind of art?))
...
?e9 (^me say-to.v ^you '(Tell me about ways art helps you cope with stress or negative emotions\.))
...
))); end of defparameter *have-arts-dialog*
...
;
(store-schema-name 'have-arts-dialog.v
                  '*have-arts-dialog*)



(setf (get '*have-arts-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-arts-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-arts-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-arts-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What types of art do you enjoy, like paintings, music, dance, or theatre?
          ((What types of art do you enjoy ?)))
   (?e5 ; Have you ever taken lessons in music, dance, theatre, drawing, or another kind of art?
          ((Have you ever taken lessons in a kind of art ?)))
   (?e9 ; Tell me about ways art helps you cope with stress or negative emotions. 
...
(setf (get '*have-arts-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-arts-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What types of art do you enjoy ?
          (art-type-you-enjoy))
   (?e5 ; Have you ever taken lessons in a kind of art ?
          (art-lessons))
   (?e9 ; How does art help you cope with negative emotions ?
          (ways-art-helps))
   )); end of mapcar
