;; *have-technology-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-technology-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-technology-dialog* ?e1). The main goal is to be able later
...
'(Event-schema (((set-of ^me ^you) have-technology-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Lets start today\'s conversation by talking about the new technology and stuff\. First tell me\. Do you have a smartphone? What do you use it for?))
...
?e5 (^me say-to.v ^you '(What is the best part of advances in technology these days?))
...
?e9 (^me say-to.v ^you '(What is the hardest part about advances in technology these days?))
...
))); end of defparameter *have-technology-dialog*
...
;
(store-schema-name 'have-technology-dialog.v
                  '*have-technology-dialog*)



(setf (get '*have-technology-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-technology-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-technology-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-technology-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Do you have a smartphone? What do you use it for?
          ((What do you use your smartphone if you have one ?)))
   (?e5 ; What is the best part of advances in technology these days?
          ((What is the best part of advances in technology ?)))
   (?e9 ; What is the hardest part of advances in technology these days?
...
(setf (get '*have-technology-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-technology-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What do you use your smartphone if you have one ?
          (smartphone-use))
   (?e5 ; What is the best part of advances in technology ?
          (technology-best-part))
   (?e9 ; What is the hardest part of advances in technology ?
