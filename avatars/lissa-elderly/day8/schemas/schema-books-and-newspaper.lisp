;; *have-books-and-newspaper-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-books-and-newspaper-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-books-and-newspaper-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-books-and-newspaper-dialog*

'(event-schema :header (((set-of ^me ^you) have-books-and-newspaper-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(I believe we talked about reading books before\. Pardon me if I do not remember the details\. Could you tell me what kinds of books you like to read?))
...
?e5 (^me say-to.v ^you '(How often do you read the newspaper?))
...
?e9 (^me say-to.v ^you '(Do you like to discuss politics and current events?))
...
))); end of defparameter *have-books-and-newspaper-dialog*
...
;
(store-schema-name 'have-books-and-newspaper-dialog.v
                  '*have-books-and-newspaper-dialog*)



(setf (get '*have-books-and-newspaper-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-books-and-newspaper-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-books-and-newspaper-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-books-and-newspaper-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What kinds of books do you like to read?
          ((What kinds of books do you like to read ?)))
   (?e5 ; How often do you read the newspaper?
          ((How often do you read the newspaper ?)))
   (?e9 ; Do you like to discuss politics and current events?
...
(setf (get '*have-books-and-newspaper-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-books-and-newspaper-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What kinds of books do you like to read ?
   ...
   (?e5 ; How often do you read the newspaper ?
          (newspaper-how-often))
   (?e9 ; Do you like to discuss politics ?
          (like-politics))
   )); end of mapcar
