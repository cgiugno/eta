...
;; *have-staying-active-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-staying-active-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-staying-active-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-staying-active-dialog*

'(event-schema :header (((set-of ^me ^you) have-staying-active-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(I visited my doctor yesterday\, and she told me that I need to be more active\. What ways have you found to stay physically active as you get older?))
...
?e5 (^me say-to.v ^you '(I like to exercise my brain as well as my body\. What are some ways you keep your brain active as you age?))
...
?e9 (^me say-to.v ^you '(What ways have you\, or someone close to you\, coped with hearing or vision loss? For example some people ask for help or put their glasses in their purse\.))
...
))); end of defparameter *have-staying-active-dialog*
...
;
(store-schema-name 'have-staying-active-dialog.v
                  '*have-staying-active-dialog*)



(setf (get '*have-staying-active-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-staying-active-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-staying-active-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-staying-active-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What ways have you found to stay physically active as you get older?
          ((How would you stay physically active ?)))
   (?e5 ; I like to exercise my brain as well as my body. What are some ways you keep your brain active as you age?
          ((How can you keep your brain active ?)))
   (?e9 ; What ways have you, or someone close to you, coped with hearing or vision loss? For example some people ask for help or put their glasses in their purse.
...
(setf (get '*have-staying-active-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-staying-active-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; How would you stay physically active ?
          (stay-physically-active))
   (?e5 ; How can you keep your brain active ?
          (keep-brain-active))
   (?e9 ; How one can cope with hearing or vision loss ? 
