...
;; *have-spirituality-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-spirituality-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-spirituality-dialog* ?e1). The main goal is to be able later
...
'(Event-schema (((set-of ^me ^you) have-spirituality-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Lets talk about spirituality\. Do you attend religious services?))
...
?e5 (^me say-to.v ^you '(Many people I\'ve talked with\, tell me they find ways to experience and express some form of spirituality either parts of organized religion or not\. What ways is spirituality a part of your life?))
...
?e9 (^me say-to.v ^you '(How does spirituality help you when life gets hard?))
...
))); end of defparameter *have-spirituality-dialog*
...
;
(store-schema-name 'have-spirituality-dialog.v
                  '*have-spirituality-dialog*)



(setf (get '*have-spirituality-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-spirituality-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-spirituality-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-spirituality-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Do you attend religious services?
          ((Do you attend religious services ?)))
   (?e5 ; Many people I've talked with... What ways is spirituality a part of your life?
          ((What ways is spirituality a part of your life ?)))
   (?e9 ; How does spirituality help you when life gets hard? 
          ((What ways is spirituality a part of your life ?)))
...
(setf (get '*have-spirituality-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-spirituality-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; Do you attend religious services ?
          (religious-services))
   (?e5 ; What ways is spirituality a part of your life ?
          (spirituality-part-of-life))
   (?e9 ; How does spirituality help you ?
