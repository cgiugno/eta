...
;; *have-home-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-home-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-home-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-home-dialog*

'(event-schema :header (((set-of ^me ^you) have-home-dialog.v) ** ?e)
;``````````````````````````````````````````````````````````````````````
...
))); end of defparameter *have-home-dialog*
...
;
(store-schema-name 'have-home-dialog.v
                  '*have-home-dialog*)



(setf (get '*have-home-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-home-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-home-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-home-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Do you live in an apartment or a house? Tell me about it.
          ((do you live in an apartment or a house ?)))
   (?e5 ; What are things you would do to make you feel comfortable in your home?
          ((What would you do to make you feel comfortable in your home ?)))
   (?e9 ; I have many memories of the home I grew up in. Could you tell me about a positive memory you have of one of your homes?  
...
(setf (get '*have-home-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-home-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; do you live in an apartment or a house ?
          (place-you-live))
   (?e5 ; What would you do to make you feel comfortable in your home ?
          (things-comfortable-home))
   (?e9 ; What is a memory you have of one of your homes ?
          (memory-from-home))
   )); end of mapcar
