;; *have-employment-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-employment-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-employment-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-employment-dialog*

'(event-schema :header (((set-of ^me ^you) have-employment-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(Retirement is a common life transition\. Tell me about the best part of the retirement that you experienced or someone you know\.))
...
?e9 (^me say-to.v ^you '(Sometimes work is one way to give back\. What are some ways you give back to your community\, others in your life\, or society as a whole?))
...
))); end of defparameter *have-employment-dialog*
...
;
(store-schema-name 'have-employment-dialog.v
                  '*have-employment-dialog*)



(setf (get '*have-employment-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-employment-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-employment-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-employment-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Think about work you have done\, it might be in the home\, a career or a volunteer work\. How did that work benefit you?
          ((How did work benefit you ?)))
   (?e5 ; Retirement is a common life transition\. Tell me about the best part of the retirement that you experienced or someone you know\.
          ((what is the best part of the retirement ?)))
   (?e9 ; Sometimes work is one way to give back. What are some ways you give back to your community, others in your life, or society as a whole?
...
(setf (get '*have-employment-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-employment-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; How could work benefit you ?
          (work-benefit))
   (?e5 ; What is the best part of the retirement ?
          (retirement-best-part))
   (?e9 ; What are some ways you give back to your community ?
