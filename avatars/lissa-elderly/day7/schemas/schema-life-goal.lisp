...
;; *have-life-goal-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-life-goal-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-life-goal-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-life-goal-dialog*

'(event-schema :header (((set-of ^me ^you) have-life-goal-dialog.v) ** ?e)
;``````````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(What makes this goal important to you? How can it improve your life quality?))
...
?e9 (^me say-to.v ^you '(What steps have you taken to achive this goal?))
...
))); end of defparameter *have-life-goal-dialog*
...
;
(store-schema-name 'have-life-goal-dialog.v
                  '*have-life-goal-dialog*)



(setf (get '*have-life-goal-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-life-goal-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-life-goal-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-life-goal-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Tell me about a personal goal you are working on to stay healthy.
          ((What is a personal goal you are working on to stay healthy ?)))
   (?e5 ; What makes this goal important to you? How can it improve your life quality?
          ((How can being healthier improve your life quality ?)))
   (?e9 ; What steps have you taken to achive this goal?
...
(setf (get '*have-life-goal-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-life-goal-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What is a personal goal you are working on to stay healthy ?
          (goal-stay-healthy))
   (?e5 ; How can being healthier improve your life quality ?
          (being-healthier-life-quality))
   (?e9 ; What steps have you taken to achieve your goal ?
          (steps-achieve-goal))
   )); end of mapcar
