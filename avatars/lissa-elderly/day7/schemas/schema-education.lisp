;; *have-education-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-education-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-education-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-education-dialog*

'(event-schema :header (((set-of ^me ^you) have-education-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Lets chat a little bit about education\. How far did you go in school?))
...
?e5 (^me say-to.v ^you '(What part of your education has been most meaningful to you?))
...
?e9 (^me say-to.v ^you '(Lifelong learning is important to me\. What do you think about lifelong learning?))
...
))); end of defparameter *have-education-dialog*
...
;
(store-schema-name 'have-education-dialog.v
                  '*have-education-dialog*)




(setf (get '*have-education-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-education-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-education-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-education-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; How far did you go in school?
          ((How far did you go in school ?)))
   (?e5 ; What part of your education has been most meaningful to you?
          ((What part of your education has been most meaningful to you ?)))
   (?e9 ; Lifelong learning is important to me. What do you think about lifelong learning?
...
(setf (get '*have-education-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-education-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; How far did you go in school?
   ...
   (?e9 ; What do you think about lifelong learning ?
