;; *have-driving-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-driving-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-driving-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-driving-dialog*

'(event-schema :header (((set-of ^me ^you) have-driving-dialog.v) ** ?e)
;`````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Now let\'s shift gears and talk about something different\. I would like to know about your experiences with cars\, you have had\, or drove at some point\. What do you remember about your first car?))
...
?e5 (^me say-to.v ^you '(I\’ve taken some great road trips\. Have you ever taken a fun road trip?))
...
?e9 (^me say-to.v ^you '(Giving up driving because of my health was very hard\. 
						I felt sad and angry\. I\’m grateful to my friends who helped me 
						cope\! Can you tell me about a way you coped with giving up 
						driving\, or a way someone you know coped?))
...
))); end of defparameter *have-driving-dialog*
...
;
(store-schema-name 'have-driving-dialog.v
                  '*have-driving-dialog*)



(setf (get '*have-driving-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-driving-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-driving-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-driving-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What do you remember about your first car?
          ((What do you remember about your first car ?)))
   (?e5 ;  Have you ever taken a fun road trip?
          ((Have you ever taken a fun road trip ?)))
   (?e9 ; Can you tell me about a way you coped with giving up driving, or a way someone you know coped?
...
(setf (get '*have-driving-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-driving-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What do you remember about your first car?
   ...
   (?e5 ; Have you ever taken a fun road trip ?
          (road-trips))
   (?e9 ; How one can cope with giving up driving ?
          (giving-up-driving))
   )); end of mapcar
