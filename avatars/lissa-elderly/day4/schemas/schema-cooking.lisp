;; *have-cooking-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-cooking-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-cooking-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-cooking-dialog*

'(event-schema :header (((set-of ^me ^you) have-cooking-dialog.v) ** ?e)
;`````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Let\'s continue our conversation by talking about cooking\. What kinds of dishes do you like to cook?))
...
?e5 (^me say-to.v ^you '(How did you learn to cook?))
...
?e9 (^me say-to.v ^you '(How have you shared cooking with people in your life?))
...
))); end of defparameter *have-cooking-dialog*
...
;
(store-schema-name 'have-cooking-dialog.v
                  '*have-cooking-dialog*)




(setf (get '*have-cooking-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-cooking-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-cooking-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-cooking-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
(setf (get '*have-cooking-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-cooking-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What kinds of dishes do you like to cook?
          (dishes-like-to-cook))
   (?e5 ; How did you learn to cook?
          (learn-to-cook))
   (?e9 ; How have you shared cooking with people in your life? 
