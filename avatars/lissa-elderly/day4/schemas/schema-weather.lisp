;; *have-weather-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-weather-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-weather-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-weather-dialog*

'(event-schema :header (((set-of ^me ^you) have-weather-dialog.v) ** ?e)
;`````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Lets start our conversation with talking a little bit about the weather\. What is the weather like outside?))
...
?e5 (^me say-to.v ^you '(Can you tell me about the forecast for this evening?))
...
?e9 (^me say-to.v ^you '(What is your favorite kind of weather?))
...
))); end of defparameter *have-weather-dialog*
...
;
(store-schema-name 'have-weather-dialog.v
                  '*have-weather-dialog*)



(setf (get '*have-weather-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-weather-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-weather-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-weather-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What is the weather like outside?
          ((How is the weather outside ?)))
   (?e5 ; Can you tell me about the forecast for this evening?
          ((How is the weather forecast for this evening ?)))
   (?e9 ; What is your favorite kind of weather?
...
(setf (get '*have-weather-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-weather-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; How is the weather outside ?
          (how-is-weather))
   (?e5 ; How is the weather forecast for this evening ?
          (weather-forecast))
   (?e9 ; What is your favorite weather ?
          (favorite-weather))
   )); end of mapcar
