;; *have-money-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-money-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-money-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-money-dialog*

'(event-schema :header (((set-of ^me ^you) have-money-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(I recently found out that I have problems with managing my money\. Probably\, because my husband used to do that before he passed away\.
	  What are some ways you manage your money? For example\, tell me about some of your strategies to pay bills on time or strategies for saving money\.))
...
?e5 (^me say-to.v ^you '(Is managing money stressful or not stressful for you?))
...
?e9 (^me say-to.v ^you '(How did you learn about managing money?))
...
))); end of defparameter *have-money-dialog*
...
;
(store-schema-name 'have-money-dialog.v
                  '*have-money-dialog*)




(setf (get '*have-money-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-money-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-money-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-money-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What are some ways you manage your money?
          ((What are some ways you manage your money ?)))
   (?e5 ; Is managing money stressful or not stressful for you?
          ((Is managing money stressful for you ?)))
   (?e9 ; How did you learn about managing money? 
...
(setf (get '*have-money-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-money-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What are some ways you manage your money ?
          (manage-money-ways))
   (?e5 ; Is managing money stressful for you ?
