;; *have-growing-older-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-growing-older-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-growing-older-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-growing-older-dialog*

'(event-schema :header (((set-of ^me ^you) have-growing-older-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(What is the hardest part of growing older?))
...
?e9 (^me say-to.v ^you '(What is the best part of growing older? ))
...
))); end of defparameter *have-growing-older-dialog*
...
;
(store-schema-name 'have-growing-older-dialog.v
                  '*have-growing-older-dialog*)



(setf (get '*have-growing-older-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-growing-older-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-growing-older-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-growing-older-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What kind of changes or life transitions might happen for you in the next year? For example, changes with your family, your health, or your living situation?
          ((What changes might happen for you in the next few years ?)))
   (?e5 ; What is the hardest part of growing older?
          ((What is the hardest part of growing older ?)))
   (?e9 ; What is the best part of growing older? 
...
(setf (get '*have-growing-older-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-growing-older-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What changes might happen for you in the next few years ?
