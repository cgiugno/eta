...
;; *have-exercise-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-exercise-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-exercise-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-exercise-dialog*

'(event-schema :header (((set-of ^me ^you) have-exercise-dialog.v) ** ?e)
;`````````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(Do you like to exercise on your own or with other people?))
...
?e9 (^me say-to.v ^you '(Do you like to exercise outdoors or in a gym?))
...
))); end of defparameter *have-exercise-dialog*
...
;
(store-schema-name 'have-exercise-dialog.v
                  '*have-exercise-dialog*)



(setf (get '*have-exercise-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-exercise-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-exercise-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-exercise-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What kinds of exercise do you do these days?
          ((What kinds of exercise do you do ?)))
   (?e5 ; Do you like to exercise on your own or with other people?
          ((Do you like to exercise alone or with other people ?)))
   (?e9 ; Do you like to exercise outdoors or in a gym?  
...
(setf (get '*have-exercise-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-exercise-dialog*))
;``````````````````````````````````````````````````````````````````
