...
;; *have-health-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-health-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-health-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-health-dialog*

'(event-schema :header (((set-of ^me ^you) have-health-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Tell me about your relationship with your primary doctor\. Do you think your doctor takes your concerns seriously?))
...
?e5 (^me say-to.v ^you '(What are the qualities of a good doctor? ))  ; what makes a good doctor?
...
?e9 (^me say-to.v ^you '(Tell me about how you are managing one of your health concerns\.))
...
))); end of defparameter *have-health-dialog*
...
;
(store-schema-name 'have-health-dialog.v
                  '*have-health-dialog*)



(setf (get '*have-health-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-health-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-health-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-health-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Tell me about your relationship with your primary doctor. Do you think your doctor takes your concerns seriously?
          ((Do you think your doctor takes your concerns seriously ?)))
   (?e5 ; What are the qualities of a good doctor?
          ((What are the qualities of a good doctor ?)))
   (?e9 ; Tell me about how you are managing one of your health concerns. 
...
(setf (get '*have-health-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-health-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; Do you think your doctor takes your concerns seriously ?
          (doctor-attitude-concerns))
   (?e5 ; What are the qualities of a good doctor ?
          (good-doctor-qualities))
   (?e9 ; how are you managing your health concerns ?
