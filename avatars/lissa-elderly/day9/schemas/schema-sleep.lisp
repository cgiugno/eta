...
;; *have-sleep-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-sleep-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-sleep-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-sleep-dialog*

'(event-schema :header (((set-of ^me ^you) have-sleep-dialog.v) ** ?e)
;``````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(I would like to talk to you about an issue I have had recently\. I don\'t sleep very well\. Sometimes I wake up several times during the night\. How well do you sleep at night? Has this changed as you\’ve gotten older?))
...
?e5 (^me say-to.v ^you '(Do you think that naps during the day are helpful? ))
...
?e9 (^me say-to.v ^you '(Tell me about something you do to improve your sleep\.))
...
))); end of defparameter *have-sleep-dialog*
...
;
(store-schema-name 'have-sleep-dialog.v
                  '*have-sleep-dialog*)



(setf (get '*have-sleep-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-sleep-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-sleep-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-sleep-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; How well do you sleep at night? Has this changed as you\’ve gotten older?
          ((How do you sleep at night ?)))
   (?e5 ; Do you think that naps during the day are helpful? 
          ((Do you think that naps during the day are helpful ?)))
   (?e9 ; Tell me about something you do to improve your sleep.
...
(setf (get '*have-sleep-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-sleep-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; How do you sleep at night ?
          (sleep-quality))
   (?e5 ; Do you think that naps during the day are helpful ?
          (opinion-about-nap))
   (?e9 ; What do you do to improve your sleep ? 
          (improve-sleep))
   )); end of mapcar
