;; *have-family-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-family-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-family-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-family-dialog*

'(event-schema :header (((set-of ^me ^you) have-family-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(I would like to learn about your family and friends\. Do you live by yourself or with others?))
...
?e5 (^me say-to.v ^you '(How long have you lived there?))
...
?e9 (^me say-to.v ^you '(Do you have children or grandchildren?))
...
?e13 (^me say-to.v ^you '(Do you use facebook or skype to keep in touch with family and friends whom you don\'t see very often? ))
...
))); end of defparameter *have-family-dialog*
...
;
(store-schema-name 'have-family-dialog.v
                  '*have-family-dialog*)




(setf (get '*have-family-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-family-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-family-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-family-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Now I’d like to learn about your family and friends. Do you live by yourself or with others?
         ((Do you live by yourself or with others ?)))
   (?e5 ; How long have you lived there?
         ((How long have you lived there ?)))
   (?e9 ; Do you have children or grandchildren?
         ((Do you have children or grandchildren ?)))
   (?e13 ; Do you use facebook or skype to be in touch with family and friends?
...
(setf (get '*have-family-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-family-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; Do you live by yourself or with others?
         (live-alone))
   (?e5 ; How long have you lived there?
         (how-long-lived-there))
   (?e9 ; Do you have children or grandchildren?
          (children))
   (?e13 ; Do you use facebook or skype?
          (use-facebook))
   	  )); end of mapcar
