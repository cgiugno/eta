;; *have-gather-together-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-gather-together-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-gather-together-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-gather-together-dialog*

'(event-schema :header (((set-of ^me ^you) have-gather-together-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(What do you usually do for holidays\, like Thanksgiving?))
...
?e5 (^me say-to.v ^you '(What is the best part?))
...
?e9 (^me say-to.v ^you '(Are there other holidays you prefer?))
...
?e13 (^me say-to.v ^you '(Have you been to any weddings recently\, or other family gatherings?))
...
))); end of defparameter *have-gather-together-dialog*
...
;
(store-schema-name 'have-gather-together-dialog.v
                  '*have-gather-together-dialog*)



(setf (get '*have-gather-together-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-gather-together-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-gather-together-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-gather-together-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What do you usually do for holidays?
         ((What do you usually do for holidays ?)))
   (?e5 ; What is the best part?
         ((What is the best part of holidays ?)))
   (?e9 ; Are there other holidays you prefer?
         ((What holidays you would prefer ?)))
   (?e13 ; Have you been to any weddings recently, or other family gatherings?
...
(setf (get '*have-gather-together-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-gather-together-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What do you usually do for holidays, like Thanksgiving?
         (holidays-activities))
   (?e5  ; What is the best part?
         (holidays-best-part))
   (?e9  ; Are there other holidays you prefer?
