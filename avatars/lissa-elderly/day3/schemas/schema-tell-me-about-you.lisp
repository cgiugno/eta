;; *have-tell-me-about-you-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-tell-me-about-you-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-tell-me-about-you-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-tell-me-about-you-dialog*

'(event-schema :header (((set-of ^me ^you) have-tell-me-about-you-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Now I would like to know more about you\. What do you think are your best qualities – the things you like best about yourself\.))
...
?e5 (^me say-to.v ^you '(What are some areas you want to grow in – things you would like to change about yourself\.))
...
?e9 (^me say-to.v ^you '(Tell me about your hopes and wishes for the future\. ))
...
))); end of defparameter *have-tell-me-about-you-dialog*
...
;
(store-schema-name 'have-tell-me-about-you-dialog.v
                  '*have-tell-me-about-you-dialog*)



(setf (get '*have-tell-me-about-you-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-tell-me-about-you-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-tell-me-about-you-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-tell-me-about-you-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What do you think are your best qualities – the things you like best about yourself?
          ((What are your best qualities ?)))
   (?e5 ; What are some areas you want to grow in – things you would like to change about yourself?
          ((what are things you want to change about yourself ?)))
   (?e9 ; : Tell me about your hopes and wishes for the future. 
...
(setf (get '*have-tell-me-about-you-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-tell-me-about-you-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What are your best qualities ?
          (your-best-quality))
   (?e5 ; what are things you want to change about yourself ?
          (want-change-about-you))
   (?e9 ; What are your hopes and wishes ?
          (hopes-and-wishes))
   )); end of mapcar
