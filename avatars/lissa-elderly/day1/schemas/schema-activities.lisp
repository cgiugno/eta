;; *have-activities-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-activities-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-activities-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-activities-dialog*

'(event-schema :header (((set-of ^me ^you) have-activities-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(I used to play tennis regularly\, but with my back\, 
...
?e9 (^me say-to.v ^you '(what kind of things you like to read?))
...
;
(store-schema-name 'have-activities-dialog.v
                  '*have-activities-dialog*)



(setf (get '*have-activities-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-activities-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-activities-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-activities-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Do you have any hobbies or anything in particular you like to do for fun? 
         ((what are your hobbies ?)))
   (?e5 ; I used to play tennis... do you like to read?
         ((do you like to read ?)))
   (?e9 ; what kind of things you like to read?
         ((what kind of things you like to read ?)))
   (?e13 ; how do you spend your days?
         ((how do you spend your days ?)))
   (?e17 ; what kind of things do you like to do in your neighborhood, like going to coffee shops, the theater, or the movies?
          ((what kind of things do you like to do in your neighborhood ?)))
   (?e21 ; Let\’s pause here so I can give you feedback.
          ((lets pause for a feedback \.)))
 )); end of mapcar #'store-output-gist-clauses


(setf (get '*have-activities-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-activities-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; do you have any hobbies..?
   ...
   (?e13 ; How do you spend your days
          (spend-your-days))
   (?e17 ; what kind of things do you like to do in your neighborhood
