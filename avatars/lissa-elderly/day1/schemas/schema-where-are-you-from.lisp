;; *have-where-are-you-from-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-where-are-you-from-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-where-are-you-from-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-where-are-you-from-dialog*

'(event-schema :header (((set-of ^me ^you) have-where-are-you-from-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Let\'s continue our conversation about your hometown\. I would like to know where you are from\.))
...
?e5 (^me say-to.v ^you '(Can you tell me more about your hometown?))
...
?e9 (^me say-to.v ^you '(How did you like the weather there?))

?e10 (^you reply-to.v ?e9) ; again leads to a (READWORDS) call, and
                             ; formation of a subordinate
                             ; (^you say-to.v ^me '(...)) plan

?e12 (^me react-to.v ?e10)

?e13  (^me say-to.v ^you '(I moved here because of my daughter\. She works here in Rochester\, so I moved here last year to be closer to her \.))

?e14 (^me say-to.v ^you '(How did you end up in Rochester?))

?e15 (^you reply-to.v ?e14)

?e16 (^me react-to.v ?e15)

?e17 (^me say-to.v ^you
      '(lets pause here so i can give you feedback))
))); end of defparameter *have-where-are-you-from-dialog*
...
;
(store-schema-name 'have-where-are-you-from-dialog.v
                  '*have-where-are-you-from-dialog*)



(setf (get '*have-where-are-you-from-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-where-are-you-from-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-where-are-you-from-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-where-are-you-from-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; where are you from?
         ((where are you from ?)))
   (?e5 ; can you tell me more about where you are from?
         ((tell me more about your hometown ?)))
   (?e9 ; How did you like the weather there?
         ((how did you like the weather in your hometown ?)))
   (?e14 ; How did you end up here ?
...
(setf (get '*have-where-are-you-from-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-where-are-you-from-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; Where are you from?
         (hometown))
   (?e5 ; Tell me more about where you are from.
         (describe-hometown))
   (?e9 ; How did you like the weather there?
          (hometown-weather))
   (?e14 ; How did you end up in Rochester?
          (endup-in-rochester))
 )); end of mapcar
