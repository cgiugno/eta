;; *have-plan-for-today-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-plan-for-today-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-plan-for-today-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-plan-for-today-dialog*

'(event-schema :header (((set-of ^me ^you) have-plan-for-today-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````
...
?e5 (^me say-to.v ^you '(What will you have for dinner tonight?))
...
?e9 (^me say-to.v ^you '(What do you like to do to wind down before bed?))
...
))); end of defparameter *have-plan-for-today-dialog*
...
;
(store-schema-name 'have-plan-for-today-dialog.v
                  '*have-plan-for-today-dialog*)



(setf (get '*have-plan-for-today-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-plan-for-today-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-plan-for-today-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-plan-for-today-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
(setf (get '*have-plan-for-today-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-plan-for-today-dialog*))
;``````````````````````````````````````````````````````````````````
...
;
 '((?e1 ; What is your plan after this session?
          (plan-after-this-session))
   (?e5 ; What will you have for dinner tonight ?
          (dinner-tonight))
   (?e9 ; What do you do to wind down before bed? 
