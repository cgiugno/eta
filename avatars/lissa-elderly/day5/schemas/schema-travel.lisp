;; *have-travel-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-travel-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-travel-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-travel-dialog*

'(event-schema :header (((set-of ^me ^you) have-travel-dialog.v) ** ?e)
;````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(What about traveling? What types of travel do you enjoy?))
...
?e5 (^me say-to.v ^you '(Tell me about your favorite vacation\.))
...
?e9 (^me say-to.v ^you '(If you won a free trip with no restrictions\, including price\, where would you go and what would you do?))
...
))); end of defparameter *have-travel-dialog*
...
;
(store-schema-name 'have-travel-dialog.v
                  '*have-travel-dialog*)



(setf (get '*have-travel-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-travel-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-travel-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-travel-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What types of travel do you enjoy?
          ((What types of travel do you enjoy ?)))
   (?e5 ;  Tell me about your favorite vacation.
          ((What does your favorite vacation look like ?)))
   (?e9 ; If you won a free trip where would you go?
...
(setf (get '*have-travel-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-travel-dialog*))
;``````````````````````````````````````````````````````````````````
