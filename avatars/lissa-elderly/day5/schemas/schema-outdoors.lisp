;; *have-outdoors-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-outdoors-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-outdoors-dialog* ?e1). The main goal is to be able later
...
?e1 (^me say-to.v ^you 
      '(Lets start our chat by talking about some outdoor activities\. I went for a walk the other day on the trails in my community\. What is something you\’ve done outdoors recently that you enjoyed?))
...
?e5 (^me say-to.v ^you '(What is your favorite season to be outdoors?))
...
?e9 (^me say-to.v ^you '(What do you enjoy doing when the weather keeps you indoors?))
...
))); end of defparameter *have-outdoors-dialog*
...
;
(store-schema-name 'have-outdoors-dialog.v
                  '*have-outdoors-dialog*)




(setf (get '*have-outdoors-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-outdoors-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-outdoors-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-outdoors-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; What is a recent outdoor activity you have done?
          ((What is a recent outdoor activity you have done ?)))
   (?e5 ; What is your favorite season to be outdoors?
          ((What is your favorite season to be outdoors ?)))
   (?e9 ; What do you enjoy doing when the weather keeps you indoors? 
...
(setf (get '*have-outdoors-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-outdoors-dialog*))
;``````````````````````````````````````````````````````````````````
