;; *have-rochester1-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-rochester1-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-rochester1-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-rochester1-dialog*

'(event-schema :header (((set-of ^me ^you) have-rochester1-dialog.v) ** ?e)
;```````````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Lets talk a little bit about Rochester\. How long have you been in Rochester?))
...
?e5 (^me say-to.v ^you '(What do you like about Rochester?))
...
?e9 (^me say-to.v ^you '(And what do you not like about it?))
...
?e13 (^me say-to.v ^you '(Is there anything you would change?))
...
))); end of defparameter *have-rochester1-dialog*
...
;
(store-schema-name 'have-rochester1-dialog.v
                  '*have-rochester1-dialog*)



(setf (get '*have-rochester1-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-rochester1-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-rochester1-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-rochester1-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; So how long have you been in Rochester?
          ((How long have you been in Rochester ?)))
   (?e5 ; What do you like most about Rochester?
          ((What do you like about Rochester ?)))
   (?e9 ; And what do you not like about it?
          ((What do you not like about Rochester ?)))
          ; ^^^^THIS INFO MAY HAVE BEEN SUPPLIED ALREADY
   (?e13 ; Is there anything you would change?
...
(setf (get '*have-rochester1-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-rochester1-dialog*))
;``````````````````````````````````````````````````````````````````
