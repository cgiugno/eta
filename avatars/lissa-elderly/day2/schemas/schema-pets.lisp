;; *have-pets-dialog*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *have-pets-dialog*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*have-pets-dialog* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defparameter *have-pets-dialog*

'(event-schema :header (((set-of ^me ^you) have-pets-dialog.v) ** ?e)
;``````````````````````````````````````````````````````````````````````
...
?e1 (^me say-to.v ^you 
      '(Now let\'s change the subject a little bit\. I have been recently thinking about getting a cat so that I spend my time with her\. I think pets are generally good for us\, especially when we get older\! Do you have a pet at home?))
...
?e5 (^me say-to.v ^you '(Tell me about a family member\'s or neighbor\'s pet\, or a pet you had previously\.))
...
?e9 (^me say-to.v ^you '(What are some ways\, pets help their owners?))
...
))); end of defparameter *have-pets-dialog*
...
;
(store-schema-name 'have-pets-dialog.v
                  '*have-pets-dialog*)



(setf (get '*have-pets-dialog* 'semantics) (make-hash-table))
 ; EL formulas (not yet used)



(mapcar #'(lambda (x) 
           (store-output-semantics (first x) (second x) '*have-pets-dialog*))
;``````````````````````````````````````````````````````````````````````
...
(setf (get '*have-pets-dialog* 'gist-clauses) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-output-gist-clauses (first x) (second x) '*have-pets-dialog*))
;``````````````````````````````````````````````````````````````````````````
...
; the next user input.
;
 '((?e1 ; Do you have a pet at home?
          ((Do you have a pet at home ?)))
   (?e5 ; Tell me about a pet you had in childhood or a pet of a family member’s.
          ((Tell me about a pet of a family member or neighbor \.)))
   (?e9 ; What are some ways pets help their owners ?
...
(setf (get '*have-pets-dialog* 'topic-keys) (make-hash-table))



(mapcar #'(lambda (x) 
           (store-topic-keys (first x) (second x) '*have-pets-dialog*))
;``````````````````````````````````````````````````````````````````
