;; TEST CONSTRUCTION OF A SMALL ARCH WITH RED UPRIGHTS AND A BLUE TOP
;;``````````````````````````````````````````````````````````````````
...
; Iterative object selections and placements start here
...
(get-matching-facts '(on B4 ?x) *kb-ht*)

(get-matching-facts '(on B2 ?x) *kb-ht*)
