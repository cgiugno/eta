; TEST SUCCESSIVE BLOCK STACKING FOR A GOAL SCHEMA WITH AN EQUALITY CONSTRAINT
; ````````````````````````````````````````````````````````````````````````````
...
(mark-objects-with-load *kb-ht*); will reflect the altered arrangement

(get 'B2 'load); ==> 2 (it supports B1 directly, B4 (in part) indirectly
