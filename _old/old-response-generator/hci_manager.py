...
5. Confirmation / Error:
	...
	def generate_response(self, query_object, answer_set, certainty):
        ...
        # IF no response:
        # 1. "There is no [red block] that is [above the NVidia block] ."
        # 2. "There are no [red blocks] that are [above the NVidia block] ."
				#  (ulf-to-english '(there.pro ((pres be.v) (no.d (n+preds (red.a block.n) (that.rel ((pres be.v) (above.p (the.d (|NVidia| block.n))))))))))
        # 3. "There is no such [block] ." (no is/are in surface form)
        #
        # IF 1 block in response relation:
        # 4. "Just [the SRI block] is [above the NVidia block] ." (question plural)
        # 5. "[the SRI block] is [above the NVidia block] ." 
        # 6. "Only [the SRI block] ."
        # 7. "I think only [the SRI block] is [above the NVidia block] ."
        # 8. "Probably just [the SRI block] is"
        #
        # IF more than 5 blocks in response relation, and query does not contain a number
        # 9. "[the SRI block] is [above the NVidia block] , but it is less certain for [the Twitter block, the Texaco block, ...] ."
        # 10. "[the SRI block and the Twitter block] are [above the NVidia block], but it is less certain for [the Texaco block] ."
        # 11. "[the SRI block] does, but it is less certain for [the Twitter block, the Texaco block, ...] ."
        # 12. "[the SRI block and the Twitter block] do, but it is less certain for [the Texaco block] ."
        # 13. "There probably are [6] [blocks] that are [above the NVidia block], possibly including [the Twitter block, the Texaco block, ...] ."
        #     (probably.adv-s (six.d (n+preds (plur block.n) (including.p (set-of ...)))))
        # 14. "There probably are [6] [blocks], possibly including [the Twitter block, the Texaco block, ...] ."
        # 15. "There are [6] that are [above the NVidia block] including [the Twitter block, the Texaco block, ...] ."
        # 16. "[The Twitter block, the Texaco block, ...] do, as well as [5] others ."

        # ELSE:
        # 17. "[the SRI block] is [above the NVidia block] , but it is less certain for [the Twitter block, the Texaco block, ...] ."
        # 18. "[the SRI block and the Twitter block] are [above the NVidia block], but it is less certain for [the Texaco block] ."
        # 19. "[the SRI block] does, but it is less certain for [the Twitter block, the Texaco block, ...] ."
        # 20. "[the SRI block and the Twitter block] do, but it is less certain for [the Texaco block] ."
        # 21. "Possibly [the SRI block and the Twitter block] are [above the NVidia block] ."
        # 22. "Possibly [the SRI block and the Twitter block] do ."
        # 23. "[the SRI block, the Twitter block, ...] are [above the NVidia block] ."
        # 24. "[the SRI block and the Twitter block] do ."
